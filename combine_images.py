...
from util import array2PIL, load_single_image
from scipy.stats import percentileofscore

parser = argparse.ArgumentParser()

parser.add_argument('-image', type=str, default='test_3.jpg')
parser.add_argument('-map', type=str, default='static/output/msroi_map.jpg')
parser.add_argument('-output_directory', type=str, default='static/output')
...
parser.add_argument('-jpeg_compression', type=int, default=20)
...
def make_quality_compression(original, sal):
    ...
    compressed = args.output_directory + '/' + 'Original_' + args.image.split('/')[-1] + '_' + str(
        args.jpeg_compression) + '.jpg'
    original.save(compressed, quality=args.jpeg_compression)

    original_size = os.path.getsize(compressed)

    uncompressed_size = os.path.getsize(args.image)

    out_img = array2PIL(k)

    if args.find_best:
        out_name = args.output_directory + '/' + 'Compressed_' + args.image.split('/')[-1] + '_' + '.jpg'

        for qual in range(90, 20, -1):
            out_img.convert('RGB').save(out_name, quality=qual)
            current_size = os.path.getsize(out_name)
            if current_size <= original_size * (1 + args.threshold_pct / 100.0):
                if args.print_metrics:
                    print(end="args.model, uncompressed_size, original_size, current_size, args.jpeg_compression, "
                              "qual, ' | '")
                break
            else:
                if args.print_metrics:
                    print(
                        end="args.model, uncompressed_size, original_size, current_size, args.jpeg_compression, qual, "
                            ", ' | '")
                pass
...
if args.single:
    ...
    for image_file in glob(image_path):
        if args.dataset == 'large':
            map_file = 'images/output_large/map' + image_file.split('/')[-1][3:-4]
        elif args.dataset == 'kodak':
            map_file = 'images/output_kodak/map_' + image_file.split('/')[-1] + '.jpg'
